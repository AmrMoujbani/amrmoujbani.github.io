...
function ArchCard({ arch, delay, inView }) {
  const [expanded, setExpanded] = useState(false)

  return (
    <div className={`reveal ${delay} ${inView ? 'visible' : ''} border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden hover:border-amber-400/50 dark:hover:border-amber-400/30 transition-all duration-300`}>
      <div className="p-6">
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          {arch.tags.map(t => (
            <span key={t} className="text-xs font-mono px-2 py-0.5 bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400 rounded">{t}</span>
          ))}
        </div>
      ...
      {expanded && (
        <div className="border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 p-6">
          {arch.imagePath ? (
            <img src={arch.imagePath} alt={`${arch.title} architecture diagram`}
              className="w-full rounded-xl object-contain max-h-[500px] cursor-zoom-in"
              onClick={() => window.open(arch.imagePath, '_blank')} title="Click to open full size" />
          ) : (
...
export default function Architectures() {
  ...
  return (
    <section ref={ref} id="architectures" className="py-24 px-6 border-t border-zinc-200 dark:border-zinc-800/60">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {architectures.map((arch, i) => (
            <ArchCard key={arch.id} arch={arch} inView={inView}
              delay={`reveal-delay-${Math.min(i + 1, 4)}`} />
          ))}
        </div>
      </div>
    </section>

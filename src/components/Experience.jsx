...
export default function Experience() {
  ...
  return (
    <section ref={ref} id="experience" className="py-24 px-6 border-t border-zinc-200 dark:border-zinc-800/60">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row gap-8">
          <div className={`reveal reveal-delay-1 ${inView ? 'visible' : ''} flex md:flex-col gap-1 overflow-x-auto md:overflow-visible shrink-0 md:w-52`}>
            {experience.map((e, i) => (
          ...
          <div className={`reveal reveal-delay-2 ${inView ? 'visible' : ''} flex-1 min-w-0`}>
            ...
            <div className="flex flex-wrap gap-2">
              {job.stack.map(t => (
                <span key={t} className="px-2.5 py-1 text-xs font-mono bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 rounded-md">{t}</span>
              ))}
            </div>
          </div>
        </div>
